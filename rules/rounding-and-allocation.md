...
- Python: bare `round(x, 2)` on a float used for money, or a `Decimal.quantize(Decimal('0.01'))` call with no explicit `rounding=` argument (it silently inherits the context default, ROUND_HALF_EVEN).
...
```
decimal_value.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)   # Python
